...
print(State,Positions)->
	%import background
    Imagebg = wxImage:new("index.jpeg"),
    Image2bg = wxImage:scale(Imagebg, wxImage:getWidth(Imagebg)*3,
			   wxImage:getHeight(Imagebg)*3),
    Bg = wxBitmap:new(Image2bg),
    wxImage:destroy(Imagebg),
    wxImage:destroy(Image2bg),
	
	%import male png
    ImageMale = wxImage:new("male.png"),
    Image2Male = wxImage:scale(ImageMale, wxImage:getWidth(ImageMale) div 20,
			   wxImage:getHeight(ImageMale) div 20),
    Male = wxBitmap:new(Image2Male),
    wxImage:destroy(ImageMale),
    wxImage:destroy(Image2Male),
	%import female png
    ImageFemale = wxImage:new("female.png"),
    Image2Female = wxImage:scale(ImageFemale, wxImage:getWidth(ImageFemale) div 20,
			   wxImage:getHeight(ImageFemale) div 20),
    Female = wxBitmap:new(Image2Female),
    wxImage:destroy(ImageFemale),
    wxImage:destroy(Image2Female),
    ...
    Fun = fun(DC) ->
		  ...
		  lists:foreach(fun({{X,Y}=Pos,Pic}) ->
					...
					case Pic of
					    bg-> wxDC:drawBitmap(DC, Bg, Pos);
					    male -> wxDC:drawBitmap(DC, Male, Pos);
					    female -> wxDC:drawBitmap(DC, Female, Pos)
						
					end
				end, [{{0,0},bg}]++Positions)
	  end,
    draw(State#state.canvas, State#state.bitmap, Fun),    
    wxBitmap:destroy(Female),
    wxBitmap:destroy(Male),
    wxBitmap:destroy(Bg).
