...
move()->
	...
	case gen_server:call(gs,{walkreq,{OldX,OldY},{OldX+MoveX,OldY+MoveY},Pid}) of
		ok->	io:format("got ok~n"),write(param,{Pid,curr},{OldX+MoveX,OldY+MoveY}),wait(1000),
		io:format("process ~p now at X=~p Y=~p~n",[Pid,OldX+MoveX,OldY+MoveY]),move();
		bar->write(param,{Pid,curr},{OldX,OldY}),wait(1000),move();
		wall->write(param,{Pid,curr},{OldX,OldY}),wait(1000),move();
		cross->ok
	end.
